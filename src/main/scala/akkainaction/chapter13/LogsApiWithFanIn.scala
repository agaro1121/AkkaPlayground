...
class LogsApiWithFanIn(implicit val mat: ActorMaterializer, ec: ExecutionContext) extends SprayJsonSupport with DefaultJsonProtocol {
      ...
      pathEndOrSingleSlash {
        ...
        path("not-ok") {
          pathEndOrSingleSlash {
            get {
              extractRequest { req =>
                complete(Marshal(fanIn.mergeNotOk(logId)).toResponseFor(req))
              }
            }
          }
        }
